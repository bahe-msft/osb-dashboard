...
func TestListSandboxesMergesLifecycleAndCustomResources(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/apis/sandbox.opensandbox.io/v1alpha1/namespaces/workloads/batchsandboxes":
			_ = json.NewEncoder(w).Encode(customResourceList{Items: []sandboxResource{
				...
				},
				{
					...
					Spec:   resourceSpec{Template: podTemplate{Spec: podSpec{Containers: []containerSpec{{Image: "batch:image"}}}}},
		...
		case "/apis/agents.x-k8s.io/v1alpha1/namespaces/workloads/sandboxes":
			_ = json.NewEncoder(w).Encode(customResourceList{Items: []sandboxResource{{
				...
				Spec:   resourceSpec{PodTemplate: podTemplate{Spec: podSpec{Containers: []containerSpec{{Image: "agent:image"}}}}},
	...
	if got := byID["batch-direct"]; got.State != "Running" || got.Namespace != "workloads" || got.PodName != "batch-direct-0" || got.Image != "batch:image" || got.Metadata["team"] != "dashboard" {
		t.Errorf("BatchSandbox = %#v", got)
	}
	if got := byID["agent-direct"]; got.State != "Running" || got.Namespace != "workloads" || got.PodName != "agent-direct-pod" || got.Image != "agent:image" || strings.Join(got.Sources, ",") != "agentsandbox" {
